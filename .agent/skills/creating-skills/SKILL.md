...
- When the user asks to generate a new capability or functionality for the agent.
- When the user mentions "Skill Creator".

## Workflow
1.  **Understand the User's Goal**: Identify the purpose of the new skill.
2.  **Determine Structure**:
    -   Folder Name: `<skill-name>/` (must be a gerund, e.g., `managing-databases`).
    -   Files: `SKILL.md` (required), optional `scripts/`, `examples/`).
3.  **Draft `SKILL.md` Content**:
    -   Frontmatter: `name` (gerund), `description` (3rd person).
    -   Body: Follow "Writing Principles" (Conciseness, Progressive Disclosure, Degrees of Freedom).
    -   Sections: Title, When to use, Workflow, Instructions, Resources.
4.  **Create Files**: Write the files to `.agent/skills/<skill-name>/`.
5.  **Confirm**: Notify the user.

## Instructions
Review the following guidelines for creating skills:
